...
export const generateEnhancedPDF = async (data: PdfData) => {
  ...
  pdf.text('ABRAJ QUIZ COMPLETE REPORT', pageWidth / 2, yPosition + 16, { align: 'center' });
  ...
  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(100, 100, 100);
  pdf.text(`Generated by Abraj Quiz System - ${currentTheme.name}`, 25, yPosition + 6);
  
  pdf.setFontSize(8);
  pdf.setFont('helvetica', 'normal');
  pdf.setTextColor(140, 140, 140);
  pdf.text(`${new Date().toLocaleString()} • Complete Interactive Learning Report`, 25, yPosition + 11);
  pdf.text('© 2025 Abraj Quiz Platform - Enhancing Education Through Interactive Technology', 25, yPosition + 15);
