...
- WebSocket origin is permissive in dev (empty `CLIENT_ORIGIN` list ⇒ allowed). If you set `CLIENT_ORIGIN` in `.env`, include `http://localhost:5000` or override with `INTEGRATION_BASE_URL` env var.
